...
public class PagerView extends FrameLayout implements ViewPager.OnPageChangeListener {
    ...
    protected void initializeFromAttributes(Context context, AttributeSet attrs) {
        TypedArray array = context.getTheme().obtainStyledAttributes(attrs, patel.jay.personal.universeweightapp.R.styleable.ExpandingCollection, 0, 0);
        try {
            this.cardWidth = array.getDimensionPixelSize(patel.jay.personal.universeweightapp.R.styleable.ExpandingCollection_cardWidth, 500);
            this.cardHeight = array.getDimensionPixelSize(patel.jay.personal.universeweightapp.R.styleable.ExpandingCollection_cardHeight, 550);
            this.cardHeaderExpandedHeight = array.getDimensionPixelSize(patel.jay.personal.universeweightapp.R.styleable.ExpandingCollection_cardHeaderHeightExpanded, 450);
        } finally {
            array.recycle();
        }
    }
    ...
    public void onPageSelected(int position) {
        ...
        BackgroundImageSwitcher.AnimationDirection direction = null;
        int nextPositionPrediction = position;
        if (oldPosition < position) {
            direction = BackgroundImageSwitcher.AnimationDirection.LEFT;
            nextPositionPrediction++;
        } else if (oldPosition > position) {
            direction = BackgroundImageSwitcher.AnimationDirection.RIGHT;
            nextPositionPrediction--;
        }
        ...
        if (attachedImageSwitcher != null) {
            ...
            BgBitmapCache instance = BgBitmapCache.getInstance();
    ...
    protected void toggleTopMargin(int duration, int delay) {
        ...
        marginAnimation.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
            ...
            public void onAnimationUpdate(ValueAnimator animation) {
                ...
                PagerView.this.setLayoutParams(containerLayoutParams);
            }
        });
    ...
    public void setBackgroundSwitcherView(BackgroundImageSwitcher imageSwitcher) {
        this.attachedImageSwitcher = imageSwitcher;
        if (imageSwitcher == null) return;
        PagerViewAdapter adapter = (PagerViewAdapter) this.pager.getAdapter();
    ...
    public void setPagerViewAdapter(PagerViewAdapter adapter) {
        this.pager.setAdapter(adapter);
        if (adapter == null) return;
        List<CardData> dataset = adapter.getDataset();
